...
public class UserController {
    ...
    public String findUserList(String keywords, Integer userListRoleId, Model model) {
        ...
        //获取用户列表
        List<User> userList = this.userService.findUserList(keywords, userListRoleId);
    ...
    public User checkLoginName(@RequestBody User user, Model model) {
        User checkUser = this.userService.getUserByLoginName(user.getLoginName());
        if (checkUser != null) {
            return checkUser;
        } else {
            checkUser = new User();
